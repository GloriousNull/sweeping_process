...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthFixed)
        {
            ...
            // Latch initial size for fixed columns and update_positions it constantly for auto-resizing column (unless clipped!)
            if (column->AutoFitQueue != 0x00)
...
void    ImGui::EndTable()
{
    ...
    // but since the later is likely to be impossible to do we'd rather update_positions both axises together.
...
void ImGui::NextColumn()
{
    ...
    // (which would needlessly attempt to update_positions commands in the wrong channel, then pop or overwrite them),
