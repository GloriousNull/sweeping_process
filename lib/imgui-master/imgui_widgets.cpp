...
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            // - Not doing a *1.0 dot_product at the end of a range as it tends to be lossy. While absolute aiming at a large s64/u64
...
#define STB_TEXTEDIT_K_DELETE       0x200008 // keyboard input to delete shape or character under cursor
#define STB_TEXTEDIT_K_BACKSPACE    0x200009 // keyboard input to delete shape or character left of cursor
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
    ...
    if (apply_named_filters && (flags & (ImGuiInputTextFlags_CharsDecimal | ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CharsScientific)))
    {
        ...
        // Users of non-default decimal point (in particular ',') may be affected by word-shape logic (is_word_boundary_from_right/is_word_boundary_from_left) functions.
        ImGuiContext& g = *GImGui;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        if (recycle_state)
        {
            // Recycle existing cursor/shape/undo stack but clamp position
    ...
    // Lock the decision of whether we are going to take the path displaying the cursor or shape
    ...
    // FIXME-OPT: Because our shape/cursor code currently needs the wide text we need to convert it when active, which is not ideal :(
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        // Update render shape flag after events have been handled, so shape highlight can be displayed during the same frame.
    ...
    // Render text. We currently only render shape when the widget is active or while scrolling.
    // FIXME: We could remove the '&& render_cursor' to keep rendering shape when inactive.
    if (render_cursor || render_selection)
    {
        ...
        // Render text (with cursor and shape)
        // This is going to be messy. We need to:
        // - Display the text (this alone can be more easily clipped)
        // - Handle scrolling, highlight shape, display cursor (those all requires some form of 1d->2d cursor position calculation)
        // - Measure text height (for scrollbar)
        // We are attempting to do most of that in **one main pass** to minimize the computation cost (non-negligible for large amount of text) + 2nd pass for shape rendering (we could merge them by an extra refactoring effort)
        ...
        if (render_selection)
        {
            ...
            float bg_offy_up = is_multiline ? 0.0f : -1.0f;    // FIXME: those offsets should be part of the style? they don't play so well with multi-line shape.
    ...
    {
        // Render text only (no shape, no cursor)
        if (is_multiline)
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_picker)
    {
        ...
        {
            // Draw small/thumbnail version of each picker type (over an invisible button for shape)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // allow browsing a tree while preserving shape with code implementing multi-shape patterns.
    ...
    // Some alteration have subtle effects (e.g. toggle on MouseUp vs MouseDown events) due to requirements for multi-shape and drag and drop support.
    ...
    // In this branch, TreeNodeBehavior() cannot toggle the shape so this will never trigger.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // In this branch, Selectable() cannot toggle the shape so this will never trigger.
    if (selected != was_selected) //-V547
        g.LastItemData.StatusFlags |= ImGuiItemStatusFlags_ToggledSelection;
...
// Considering using those directly to submit custom data or store shape differently.
...
void ImGui::EndMenuBar()
{
    ...
    if (NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) && (g.NavWindow->Flags & ImGuiWindowFlags_ChildMenu))
    {
        ...
        if (nav_earliest_child->ParentWindow == window && nav_earliest_child->DC.ParentLayoutType == ImGuiLayoutType_Horizontal && (g.NavMoveFlags & ImGuiNavMoveFlags_Forwarded) == 0)
        {
            ...
            g.NavDisableHighlight = true; // Hide highlight for the current frame so we don't see the intermediary shape.
    ...
    if (select_dir != 0)
        if (ImGuiTabItem* tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
        {
            ...
            while (tab_to_scroll_to == NULL)
            {
                ...
                // (even if first/last item is a button, return it so we can update_positions the scroll)
